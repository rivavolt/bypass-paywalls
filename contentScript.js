              ...
              if (body) {
                ...
                for (let par of pars) {
                  if (par.tagName === 'p') {
                    if (par_first)
                      par_first = false;
                    let doc = parser.parseFromString('<p class="' + par_class + '" data-type="paragraph">' + DOMPurify.sanitize(par.innerHTML.replace(/(<\/?mark([^>]+)?>)/g, ''), dompurify_options) + '</section>', 'text/html');
                    par_new = doc.querySelector('p');
                    if (par_new) {
